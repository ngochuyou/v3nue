...
export const UPDATE_MODEL = "SPECZ_UPDATE_MODEL";
export const UPDATE_LIST = "SPECZ_UPDATE_LIST";
...
export function createSpecialization(model) {
	if (!model) {
		return Result.error(null, "Model can not be empty", "client-400");
	}

	return fetch(`${server.url}/api/factor/specialization`, {
		method: 'POST',
		mode: 'cors',
		headers: {
...
export function editSpecialization(model) {
	if (!model) {
		return Result.error(null, "Model can not be empty", "client-400");
	}
